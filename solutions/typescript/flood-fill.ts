 ...
 * (sr, sc) = (1, 1) (i.e., the red pixel), all pixels connected by a 
 * path of the same color as the starting pixel (i.e., the blue pixels) 
 ...
 * @return {number[][]}
 */
var floodFill = function(image:number[][], sr:number, sc:number, color:number):number[][] {
  ...
  // check bottom
  if(sr + 1 < image.length && image[sr + 1][sc] === originalColor) floodFill(image, sr + 1, sc, color);
  // check left
  if(sc - 1 >= 0 && image[sr][sc - 1] === originalColor) floodFill(image, sr, sc - 1, color);
  // check right
  if(sc + 1 < image[sr].length && image[sr][sc + 1] === originalColor) floodFill(image, sr, sc + 1, color);
